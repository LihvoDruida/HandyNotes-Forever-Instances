...
CURRENT_VERSION=$(sed -n 's/^## Version:[[:space:]]*v\{0,1\}//p' "Forever_Instances_Camelot.toc" | head -n1 | tr -d '\r')
...
UNEXPECTED=$(git status --porcelain --untracked-files=all | sed -E 's/^.. //' | grep -Ev '^("?Forever_Instances_Camelot\.toc"?|CHANGELOG\.md)$' || true)
...
git add "Forever_Instances_Camelot.toc" CHANGELOG.md
